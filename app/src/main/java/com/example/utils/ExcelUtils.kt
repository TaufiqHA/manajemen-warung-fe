...
fun generateMenuCetakExcel(context: Context, menuList: List<MenuItem>, categoryOrder: List<String>) {
    try {
        ...
        showToast(context, "File Excel berhasil dibuat")

        // 7. Buka file secara otomatis setelah berhasil dibuat
        openExcelFile(context, file)
    } catch (e: Throwable) {
        e.printStackTrace()
        showToast(context, "Gagal membuat Excel. Mungkin perangkat tidak mendukung format ini. Error: ${e.message}", Toast.LENGTH_LONG)
    }
}
...
fun openExcelFile(context: Context, file: File) {
    try {
        ...
        // Tangkap error jika pengguna tidak punya aplikasi Excel
        showToast(context, "Aplikasi pembaca Excel tidak ditemukan. File berhasil disimpan di folder Documents.", Toast.LENGTH_LONG)
    } catch (e: Exception) {
        showToast(context, "Gagal membuka file Excel: ${e.message}")
    }
}

/**
 * Helper to safely show toast on the UI thread
 */
